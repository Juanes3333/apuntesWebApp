...
@Component
public class DeliveryRecordServlet extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
        ...
        List<DeliveryRecord> deliveryRecords = fleetService.getAllDeliveryRecords();
        ...
        for (DeliveryRecord deliveryRecord : deliveryRecords) {
            out.println("<tr>");
            out.println("<td>" + deliveryRecord.getId() + "</td>");
            out.println("<td>" + deliveryRecord.getTrackingCode() + "</td>");
            out.println("<td>" + deliveryRecord.getPackageDescription() + "</td>");
            out.println("<td>" + deliveryRecord.getDeliveryDate() + "</td>");
            out.println("<td>" + deliveryRecord.getDestination() + "</td>");
            out.println("<td>" + deliveryRecord.getPriorityLevel() + "</td>");
            out.println("<td>" + deliveryRecord.getVehicleId() + "</td>");
            out.println("</tr>");
        }
        out.println("</table>");
    ...
    // POST /deliveries: lee los parámetros del formulario, arma un DeliveryRecord y delega la
    // validación/registro en fleetService.registerDeliveryRecord (las reglas de negocio viven ahí).
    @Override
    protected void doPost(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
        ...
        int newId = fleetService.getAllDeliveryRecords().size() + 1;
        DeliveryRecord deliveryRecord = new DeliveryRecord(newId, trackingCode, packageDescription,
                deliveryDate, destination, priorityLevel, vehicleId);

        boolean registered = fleetService.registerDeliveryRecord(deliveryRecord);

        resp.setContentType("text/html;charset=UTF-8");
        PrintWriter out = resp.getWriter();
        out.println("<html><body>");
        if (registered) {
            out.println("<p>Delivery record registered successfully.</p>");
        } else {
            out.println("<p>Delivery record could not be registered (invalid data, duplicate tracking "
                    + "code, or vehicle not available).</p>");
        }
        out.println("<a href='deliveries'>Back to delivery record list</a>");
        out.println("</body></html>");
    }
}
